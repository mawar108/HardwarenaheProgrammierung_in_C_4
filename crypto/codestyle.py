...
def check_disallowed(args, code):
    identifiers = set(re.findall(IDENTIFIER, code))

    for disallowed in DISALLOWED.strip().split("\n"):
        if re.match(IDENTIFIER, disallowed):
            if disallowed in identifiers:
                raise CodeStyleError(f'"{disallowed}" not allowed')
        else:
